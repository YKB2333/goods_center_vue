        ...
        <div class="mb22 plr16" v-if="showBrandCategory">
          ...
          <div class="row flex-wrap">
            <div 
              ...
              :class="selectBrandCategory.some(value => value === item.type) ? 'active' : ''" 
        ...
        <div class="mb22 plr16" v-if="showBrand && brandList.length">
          ...
          <div class="row" style="flex-wrap: wrap;">
            <div 
              ...
              :class="brandSelectList.some(value => value === item.id) ? 'active' : ''"
...
export default {
  ...
  created() {
    this.filter = {
      workStatus: this.$route.query.workStatus||'', // 合作状态
      lowerMsrp: this.$route.query.lowerMsrp||'', // 最低价
      higherMsrp: this.$route.query.higherMsrp||'', // 最高价
      isGeneralTrade: this.$route.query.isGeneralTrade||false, // 一般贸易
      isCrossBorderTrade: this.$route.query.isCrossBorderTrade||false, // 跨境贸易
      brandCategorys: this.$route.query.brandCategorys||[], // 商品服务类型
      brandIds: this.$route.query.brandIds||[], // 品牌id
      cateOne: this.$route.query.cateOne||''
    }
    if(!Array.isArray(this.filter.brandCategorys)){
      this.filter.brandCategorys = this.filter.brandCategorys.split('')
    }
    this.brandSelectList = this.$route.query.brandIds || []
    this.selectBrandCategory = this.$route.query.brandCategorys || []
  },
  methods: {
    showFilter() {
    ...
    onBrandSelect(item) {
      let index = this.brandSelectList.findIndex(value => value === item.id)
      if (index === -1) {
        this.brandSelectList.push(item.id)
      } else {
    ...
    onBrandCategoryClick(item) {
      ...
      let index = this.selectBrandCategory.findIndex(value => value === item.type)
      if (index === -1) { // 不存在
        this.selectBrandCategory.push(item.type)
      } else {
    ...
    onConfirm() {
      ...
      console.info('onConfirm', this.$route.query['cateOne'])
      ...
      data['brandIds'] = this.brandSelectList.map(item => item)
      // data['brandCategorys'] = this.selectBrandCategory.map(item => item)
      console.log('slee', this.selectBrandCategory)
      data['brandCategorys'] = this.selectBrandCategory.length>0?this.selectBrandCategory.map(item => item):this.$route.query.brandCategorys?this.$route.query.brandCategorys:this.$route.query.brandCategory?this.$route.query.brandCategory:[]
    ...
    resetFilterValue() {
      ...
      if(this.$route.query.brandCategory){
        this.filter['brandCategorys'].push(this.$route.query.brandCategory)
      }else if(this.$route.query.showBrandCategorys){
        this.filter['brandCategorys'] = this.$route.query.brandCategorys?this.$route.query.brandCategorys:this.$route.query.brandCategory?this.$route.query.brandCategory:[]
      }else{
